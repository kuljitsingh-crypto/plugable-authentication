...
const descriptionName = "#VerifyTokenWithDefaultOption";

describe(descriptionName, function () {
  const shouldExecute = isSingleTestFileExceutionMatchFile(descriptionName);
  if (shouldExecute) {
    let csrfToken = null;
    context("verify use cookie token with default options ", function () {
      ...
      it("should throw error 'Looks like you tried to connect with a different IP Address.'", function (done) {
        ...
        express
          ...
          .then(() => {
            express2 = dummyExpress({ csrfToken, userIp: userIpAddress.ip2 });
